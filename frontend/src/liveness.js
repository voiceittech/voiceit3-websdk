...
export default function Liveness(VoiceItObj) {
  ...
  LivenessRef.setupVariables = function(testType) {
    LivenessRef.tests = vi$.shuffle([0, 1, 2]); // TODO: 3 Smile Disabled 0 Down and 4 Yawn Tests Disabled
  ...
  LivenessRef.startLiveness = function(testType) {
    if (LivenessRef.livenessStarted) {
      return;
    }
  ...
  LivenessRef.livenessCheck = function(pose) {
    const faceObject = {
      ...
      eyeMidPoint:
        (pose.keypoints[2].position.x - pose.keypoints[1].position.x) / 2.0 +
        pose.keypoints[1].position.x,
      eyeMidHeight:
        (pose.keypoints[2].position.y + pose.keypoints[1].position.y) / 2.0,
      earMidHeight:
        (pose.keypoints[3].position.y + pose.keypoints[4].position.y) / 2.0
    };
        ...
        if (LivenessRef.passed.value) {
          ...
          if (LivenessRef.passedTests < LivenessRef.numTests) {
            ...
            if (LivenessRef.type === "face") {
              ...
              LivenessRef.type === "video" &&
              LivenessRef.continueToVoice
            ) {
